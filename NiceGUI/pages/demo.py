    ...
    with (
                ...
                with ui.column().classes(
                    ...
                    async def toggle_recording():
                        if not is_recording["value"]:
                            try:
                                await ui.run_javascript("await baseeraStartRecording()")
                            ...
                            record_btn.set_text("🎙️ START VOICE RECORDING")
                            record_btn.classes(remove="recording-active")
                            rec_status.set_text("Voice captured and processed successfully!")
        ...
        with ui.card().classes(
            ...
            with ui.row().classes("w-full gap-6 items-start"):
                with ui.column().classes("items-center").style("flex: 7 1 0%; min-width: 0;"):
                    ui.label("Live Web Camera Stream").classes("text-xs font-semibold text-slate-500 mb-2")
                    ui.html(
                    ...
                    ui.button("Snap Camera Frame", icon="photo_camera", on_click=capture_frame).classes(
                        "mt-4 px-6 py-2.5 text-xs font-bold rounded-xl cursor-pointer"
        ...
        with ui.column().classes("w-full items-center my-4"):
            ...
            async def run_pipeline():
                ...
                ui.notify(
                    "Processing scene & voice query... this can take a while on the first "
                    "request or on a CPU-only machine (Whisper + YOLO + the local LLM + "
                    "text-to-speech all run in one request).",
                    type="info",
                    timeout=8000,
                )
                ...
                try:
                    # Generous timeout: on CPU-only hardware, Whisper + two YOLO models +
                    # the local LLM (called twice) + gTTS in a single request can genuinely
                    # take a couple of minutes, especially on the very first request after
                    # the backend just finished loading models. A short timeout here used to
                    # get caught by the broad `except Exception` below and reported as
                    # "Failed to connect to backend server" — misleading, since the backend
                    # had in fact received the request and was still working on it.
                    ...
                    ui.notify(
                        "The backend received the request but took longer than 3 minutes to "
                        "respond, so the frontend gave up waiting. This usually means it's "
                        "still genuinely processing (check the backend terminal — CPU-only "
                        "inference of Whisper + YOLO + the local LLM can be slow), not that "
                        "it's unreachable. Wait for the backend log to finish, then try again "
                        "— later requests are faster once everything is warmed up.",
                        type="negative",
                    )
            ...
            ui.button("SEE DASHBOARD RESULTS", icon="dashboard", on_click=run_pipeline).classes(
